...
function try_find_root_eff!(
  ...
  function j!(J, x)
    ...
    hessian_n = autodiff_hessian(potential_eff, flds_n, params, (R1, R2), parsample)
    hessian_cb = autodiff_hessian(potential_eff, flds_cb, params, (R1, R2, C1), parsample)
    for i in 1:5
      J[1, i] = hessian_n[1, i]
      J[2, i] = hessian_n[2, i]
...
function solve_tree_eqns!(nvac::Vacuum, cbvac::Vacuum, params::Params{Float64})
    ...
    coefs1 = [nv1,-nv2,0,nv1^3/2.,0,(nv1*nv2^2)/2.,(nv1*nv2^2)/2.,(nv1*nv2^2)/2.]
    coefs2 = [0,-nv1,nv2,0,nv2^3/2.,(nv1^2*nv2)/2.,(nv1^2*nv2)/2.,(nv1^2*nv2)/2.]
    coefs3 = [cv1,-cv2,0,(4*cv1^3 + 4*cv1*cv3^2)/8.,0,(cv1*cv2^2)/2.,(cv1*cv2^2)/2.,(cv1*cv2^2)/2.]
    coefs4 = [0,-cv1,cv2,0,cv2^3/2.,(4*cv1^2*cv2 + 4*cv2*cv3^2)/8.,(cv1^2*cv2)/2.,(cv1^2*cv2)/2.]
    coefs5 = [cv3,0,0,(4*cv1^2*cv3 + 4*cv3^3)/8.,0,(cv2^2*cv3)/2.,0,0]
